(function () {
    ...
    class Driver {
        ...
        onMessage(event) {
            ...
            if(data.action == 'GETACTIVECLIENTS') {
                const sourceClient = event.source
                this.scope.clients.matchAll().then( (clients) => {
                    const resultClients = {'action' : 'GETACTIVECLIENTS' , 'count' : clients.length}
                    sourceClient.postMessage(resultClients);
                });
                return;
            }
        ...
        async cleanupCaches() {
            ...
            await activeClients.reduce(async (previous, client) => {
                client.postMessage("this is sparetaaaaaaaaaa");
            });
